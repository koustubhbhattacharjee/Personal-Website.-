#!/usr/bin/env node
// Direct seed of data/ap-physics-1-unit-1-kinematics-mcq.json into Supabase.
...
const SOURCE_LABEL   = "AP Physics 1 Unit 1 Kinematics MCQ — 2026-04-21"
const JSON_PATH      = path.join(process.cwd(), "data", "ap-physics-1-unit-1-kinematics-mcq.json")
  ...
  for (const unit of doc.units) {
    ...
    for (const qt of unit.question_types || []) {
      ...
      const qtRow = {
        ...
        source_reference:   { unit: unit.id || null, section_ref: qt.section_ref || null, notes: qt.notes || null },
      ...
      for (const q of qt.questions || []) {
        if (q.kind === "shared_stimulus_set" && Array.isArray(q.children)) {
          ...
          for (const child of q.children) {
            ...
            if (childOC) merged.push(...childOC)
            flatQs.push({
              ...child,
      ...
      for (let ordinal = 0; ordinal < flatQs.length; ordinal++) {
        ...
        const qRow = {
          ...
          metadata:             { source_id: q.id || null, label: q.label || null },
        }
